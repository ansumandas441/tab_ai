...
async function main() {
  ...
  // ── Step 2: Fetch optional history context for relevant commands ─────────
  let historyContext = '';
  ...
  if (config.debug) {
    ...
    console.log(chalk.magenta(`  ${tabs.length} tabs fetched`));
    console.log(chalk.magenta(`  Mode: ${config.pipeline !== false ? 'pipeline' : 'legacy'}`));
  ...
  if (config.pipeline !== false) {
    // ── Pipeline mode (classify → resolve → emit) ─────────────────────
    try {
      action = await runPipeline({
    ...
    // ── Legacy mode (single monolithic queryOllama call) ──────────────
